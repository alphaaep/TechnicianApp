jQuery(window).load(function () {
    ...
    $(window).resize(function () {
        ...
        if (windowsize < 960) {
            if (!body.hasClass('left_nav_hide')) {
                bodyy.addClass('left_nav_hide');
                if (bodyy.hasClass('left_nav_fixed')) {
                    bodyy.removeClass('left_nav_fixed');
                    left_nav = 1;
                }
                calculateHeight();
            }
    ...
    $(".btn-mini").click(function () {
        ...
        if ($(this).hasClass('btn-primary')) {
            ...
            bodyy.removeClass($td.eq(1).text());
    ...
    $('.left_nav_slidebar ul li').on('click', function (e) {
        ...
        if ($(this).has('ul').length) {
            ...
            if (current_class == 'opened') {
                ...
                var lnsul = $('.left_nav_slidebar ul li');
                lnsul.find('ul:visible').parent().removeClass('left_nav_active theme_border');
                lnsul.find('ul:visible').slideToggle("normal");
                lnsul.find('ul:visible').removeClass("opened");
